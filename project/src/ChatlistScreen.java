	...
	private void requestChatRoomList() {
	    try {
	        ...
	        if (chatRoomList != null && !chatRoomList.isEmpty()) {
	            // 채팅방 목록 정렬 및 필터링
	        	String[] rooms = chatRoomList.split("::"); //채팅방 문자열을 "::"로 분리
	            List<String> filteredRooms = new ArrayList<>(); //결과를 저장할 리스트
	            Set<String> seenRooms = new HashSet<>(); //중복 체크를 위한 Set
	            
	            for (String room : rooms) {
	                // 참가자를 쉼표로 나누고 알파벳 순으로 정렬
	                String[] participants = room.split(", ");
	                Arrays.sort(participants);
	                
	                // 정렬된 참가자 목록을 다시 문자열로 합침
